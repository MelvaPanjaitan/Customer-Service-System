...
WebUI.click(findTestObject('Object Repository/Antrian Online/Daftar Antrian/Approve/Approve'))

WebUI.acceptAlert()

for (int row = 1; row <= excelMelva.getRowNumbers(); row++) {
    if (excelMelva.getValue('input', row) == 'YES') {
        if (findTestData('Data Files/Excel TSSPV').getValue('ditangani sendiri', row) == 'YES') {
        ...
        WebUI.click(findTestObject('Object Repository/Antrian Online/Form Penanganan/Status Penyelesaian-DDL/Status Penyelesaian'))

        WebUI.click(findTestObject('Object Repository/Antrian Online/Form Penanganan/Status Penyelesaian-DDL/Close ' + excelMelva.getValue(
                    'status penyelesaian', row)))
